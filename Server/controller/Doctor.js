...
exports.signup = async (req, res) => {
    try {
        const {firstName, lastName, email, password, phoneNo, role, type, licenseNo, state} = req.body;
        if(!firstName || !lastName || !email || !password || !phoneNo || !role || !type || !licenseNo) {
        ...
        await Doctor.create({
            firstName, lastName, email, password: hashedPassword, phoneNo, role, type, licenseNo, image: response.secure_url, state
        })
        return res.status(200).json({
            success: true,
...
exports.getDoctorDetails = async(req, res) => {
    try {
        ...
        const doctor = await Doctor.findOne({_id : new mongoose.Types.ObjectId(decode.id)});
        return res.status(200).json({
            success: true,
            data: doctor,
...
exports.getAppointments = async(req, res) => {
    try {
        ...
        const appointments = await Appointment.find({doctor: new mongoose.Types.ObjectId(decode.id)}).sort({date: -1}).populate('user').populate('slot').exec();
        return res.status(200).json({
            success: true,
            data: appointments,
...
exports.updateProfilePicture = async(req, res) => {
    try {
        ...
        const response = await uploadFileToCloudinary(file,'Whisker Whisper');
        await Doctor.findByIdAndUpdate({_id: new mongoose.Types.ObjectId(decode.id)},{
...
exports.addSlot = async(req, res) => {
    try {
        ...
        const {time,street,city,state,postalCode,bookingsAccepted,date,day} = req.body;
        const slot = await Slot.create({time, street, city, state, postalCode, bookingsAccepted, date,day, doctor: decode.id});
        await Doctor.findByIdAndUpdate({_id: new mongoose.Types.ObjectId(decode.id)},{
            $push: {
                slots: slot._id
            }
