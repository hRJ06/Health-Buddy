...
exports.signup = async (req, res) => {
    try {
        ...
        await User.create({
            firstName, lastName, email, password: hashedPassword, phoneNo, role, image: `https://api.dicebear.com/6.x/initials/svg?seed=${firstName.charAt(0)+lastName.charAt(0)}`
        })
        return res.status(200).json({
            success: true,
...
exports.addPet = async(req, res) => {
    try {
        ...
        await User.findByIdAndUpdate({_id : new mongoose.Types.ObjectId(decode.id)},{
            $push: {
                pets: pet._id
            }
        })
...
exports.bookAppointment = async (req, res) => {
    try {
        ...
        let slot = await Slot.findById({_id: new mongoose.Types.ObjectId(slotId)});
        ...
        await mailSender(user.email, 'Appointment Booked', await AppointmentBookedUser(appointment, slot));
        await mailSender(doctor.email, 'New Appointment', await AppointmentBookedDoctor(appointment, user, slot));
...
exports.getAllPets = async(req, res) => {
    try {
        ...
        const user = await User.findById({_id: new mongoose.Types.ObjectId(decode.id)})
        .populate({
            path: 'pets',
            populate: {
                path: 'owner'
...
exports.AddToCart = async(req, res) => {
    try {
        ...
        const cartItem = await Cart.findOne({productId: productId, buyer: decode.id});
        if(cartItem) {
            return res.status(200).json({
                success: false,
        ...
        const cart = await Cart.create({productId, buyer: decode.id, insertTime: Date.now()});
        await User.findByIdAndUpdate({_id: new mongoose.Types.ObjectId(decode.id)}, {
            $push: {
                cartItems: cart._id
            }
...
const checkExpiredCartItems = async(req, res) => {
    try {
        ...
        const expiredCarts = await Cart.find({insertTime: {$lt: twentyFourHoursAgo}, emailSent: false});
        if(expiredCarts.length > 0) {
            expiredCarts.forEach(async(cart) => {
                const buyer = cart.buyer;
                const item = cart.productId;
                const product = await Product.findById({_id: new mongoose.Types.ObjectId(item)});
                const user = await User.findById({_id: new mongoose.Types.ObjectId(buyer)});
                await mailSender(user.email,'Your Item is Waiting in the Cart - Complete Your Booking Now!',await CompleteBooking(product,user));
            })
        }
    }
    catch (err) {
...
exports.getAppointments = async(req, res) => {
    try {
        ...
        const user = await User.findById({ _id: new mongoose.Types.ObjectId(decode.id) }).populate({path: 'appointments', populate: {path: 'slot'}});
...
exports.cancelAppointment = async(req, res) => {
    try {
        ...
        const appointment = await Appointment.findByIdAndUpdate({_id: new mongoose.Types.ObjectId(appointmentId)},
           {
            status: 'Cancelled',
           }
        )
        const user = await User.findById({_id: new mongoose.Types.ObjectId(decode.id)});
        const slot = await Slot.findByIdAndUpdate({_id: new mongoose.Types.ObjectId(appointment.slot)}, {
            $inc: {
                currentBookings : -1,
            }
        })
        await mailSender(user.email,'Appointment Cancelled',await CancelAppointment(appointment,user,slot));
        return res.status(200).json({
            success: true,
...
const checkAppointments = async () => {
    ...
    try {
      const pendingAppointments = await Appointment.find({
        status: 'Pending',
      }).populate('slot').populate('user').populate('doctor');
  
      pendingAppointments.forEach(async (appointment) => {
        const appointmentDateStr = appointment.slot.date; 
        const [appointmentDay, appointmentMonth, appointmentYear] = appointmentDateStr.split('/');
        const appointmentDate = new Date(`${appointmentMonth}/${appointmentDay}/${appointmentYear}`);
        ...
        if (currentDate > appointmentDate) {
          ...
          await mailSender(appointment.user.email, 'Appointment Resolved', await AppointmentResolved(appointment, appointment.user, appointment.doctor));
        }
      });
    } catch (error) {
...
exports.getSlots = async (req, res) => {
    try {
        ...
        const slots = await Slot.find({doctor: new mongoose.Types.ObjectId(doctorId)}).populate('doctor').exec();
        const geocodedSlots = await Promise.all(
            slots.map(async (slot) => {
                ...
                const options = {
                    ...
                    headers: {
                        'X-RapidAPI-Key': '5226abccf0msha3b1b6c852bc943p13cb3bjsn6457c63a193f',
                        'X-RapidAPI-Host': 'trueway-geocoding.p.rapidapi.com'
                    }
                };
                try {
                    const geocodingResponse = await axios.request(options);
                    const { lat: slotLat, lng: slotLng } = geocodingResponse.data.results[0].location;
...
exports.addReview = async(req, res) => {
    try {
        ...
        const appointment = await Appointment.findById({_id: new mongoose.Types.ObjectId(appointmentId)}).populate('doctor').exec();
        await Doctor.findByIdAndUpdate({_id: appointment.doctor._id}, {
            $inc: {
                review: review,
                reviewCount: 1
            }
